...
/*
 * KUnit function redirection (kprobe stubbing) API.
 */
...
struct kunit_kprobe_stub_ctx {
	unsigned long real_fn_addr;
	unsigned long replacement_addr;
	struct kprobe kp;
};

static void __kunit_kprobe_stub_resource_free(struct kunit_resource *res)
{
	struct kunit_kprobe_stub_ctx *ctx = res->data;
...
{
	struct kunit_kprobe_stub_ctx *ctx = container_of(kp, struct kunit_kprobe_stub_ctx, kp);
...
static bool __kunit_kprobe_stub_resource_match(struct kunit *test,
					       struct kunit_resource *res,
					       void *match_real_fn_addr)
{
	struct kunit_kprobe_stub_ctx *ctx = res->data;

	/* Make sure the resource is a kprobe stub resource. */
	if (res->free != &__kunit_kprobe_stub_resource_free)
		return false;
...
{
	struct kunit_kprobe_stub_ctx *ctx;
	struct kunit_resource *res;
	...
	if (!replacement_addr) {
		kunit_deactivate_kprobe_stub(test, real_fn_addr);
		return;
	}

	/* Look up any existing stubs for this function, and replace them. */
	res = kunit_find_resource(test,
				  __kunit_kprobe_stub_resource_match,
				  real_fn_addr);
	if (res) {
		ctx = res->data;
		...
		kunit_alloc_resource(test, NULL,
				     __kunit_kprobe_stub_resource_free,
				     GFP_KERNEL, ctx);
	}
}
EXPORT_SYMBOL_GPL(__kunit_activate_kprobe_stub);
...
EXPORT_SYMBOL_GPL(kunit_deactivate_kprobe_stub);
